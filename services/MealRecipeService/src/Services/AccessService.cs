...
public class AccessService : Interfaces.IAccessService
{
    ...
    public async Task<Result<IEnumerable<ResourcePermissionResponse>>> GetSharedMeals(Guid mealId)
    {
        var permissions = await _resourcePermissonsRepository.GetByResourceTypeNameAsync(mealId, "meal");
        return Result<IEnumerable<ResourcePermissionResponse>>.Success(permissions.Select(ToResponse));
    }

    public async Task<Result<IEnumerable<ResourcePermissionResponse>>> GetSharedRecipes(Guid recipeId)
    {
        var permissions = await _resourcePermissonsRepository.GetByResourceTypeNameAsync(recipeId, "recipe");
        return Result<IEnumerable<ResourcePermissionResponse>>.Success(permissions.Select(ToResponse));
    }

    public async Task<Result<IEnumerable<ResourcePermissionResponse>>> GetMealsSharedWithUser(Guid userId)
    {
        var permissions = await _resourcePermissonsRepository.GetBySubjectAndResourceTypeNamesAsync(userId, "user", "meal");
        return Result<IEnumerable<ResourcePermissionResponse>>.Success(permissions.Select(ToResponse));
    }

    public async Task<Result<IEnumerable<ResourcePermissionResponse>>> GetRecipesSharedWithUser(Guid userId)
    {
        var permissions = await _resourcePermissonsRepository.GetBySubjectAndResourceTypeNamesAsync(userId, "user", "recipe");
        return Result<IEnumerable<ResourcePermissionResponse>>.Success(permissions.Select(ToResponse));
    }
